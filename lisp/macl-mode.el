;;; macl-mode.el --- Lisp mode, and its idiosyncratic commands  -*- lexical-binding:t -*-
...
(define-abbrev-table 'macl-mode-abbrev-table ()
...
(defvar macl-mode-syntax-table
  (let ((table (make-syntax-table lisp-data-mode-syntax-table)))
    ...
    table)
  "Syntax table used in `macl-mode'.")

(rx-define macl-mode-symbol (+ (| (syntax word)
                                  (syntax symbol)
                                  (: "\\" nonl))))

(eval-and-compile
  (defconst macl-mode-symbol-regexp (rx macl-mode-symbol)))
...
(defconst macl-mode-autoload-regexp
  "^;;;###\\(\\([-[:alnum:]]+?\\)-\\)?\\(autoload\\)"
...
(put 'defrecord 'doc-string-elt 2)
          ...
          (let* ((ppss (syntax-ppss))
            ...
            (pcase parent
              ...
              ('let
                (forward-sexp 1)
                (>= pos (point)))
              ((or 'defun 'defmacro 'cl-defobfun 'cl-defun)
                (forward-sexp 2)
                (>= pos (point)))
              ('condition-case
...
(defun lisp--el-match-keyword (limit)
  ;; FIXME: Move to emacl-mode.el.
  (catch 'found
    (while (re-search-forward
            (concat "(\\(" (rx macl-mode-symbol) "\\)\\_>")
            limit t)
...
(defun macl-mode--search-key (char bound)
  (catch 'found
    (while (re-search-forward
            (concat "\\_<" char (rx macl-mode-symbol) "\\_>")
            bound t)
...
(let-when-compile
    ((lisp-fdefs '("defmacro" "defun"))
     ...
     (eieio-fdefs '("defgeneric" "defobfun"))
     (eieio-tdefs '("defobject" "defrecord"))
     ;; Common-Lisp constructs supported by cl-lib.
     (cl-lib-fdefs '("defmacro" "defsubst" "defun" "defobfun" "defgeneric"))
     (cl-lib-tdefs '("defstruct" "deftype" "defrecord"))
  ...
  (let ((vdefs (eval-when-compile
    ...
    (defconst lisp-el-font-lock-keywords-1
      `( ;; Definitions.
        (,(concat "(" el-defs-re "\\_>"
          ...
          (3 (let ((type (get (intern-soft (match-string 1)) 'lisp-define-type)))
               (cond ((eq type 'var) font-lock-variable-name-face)
                     ...
                     ;; defobfun with (setf foo) as name.
        ...
        (,macl-mode-autoload-regexp (3 font-lock-warning-face prepend)
    ...
    (defconst lisp-el-font-lock-keywords-2
      (append
       ...
       `( ;; Regexp negated char group.
         ...
         (,(rx "\\\\" (or (seq "[" (group-n 1 macl-mode-symbol) "]")
                          (seq "`" (group-n 1
                                     ;; allow multiple words, e.g. "C-x a"
                                     macl-mode-symbol (* " " macl-mode-symbol))
                               "'")))
          (1 font-lock-constant-face prepend))
         (,(rx "\\\\" (or (seq "<" (group-n 1 macl-mode-symbol) ">")
                          (seq "{" (group-n 1 macl-mode-symbol) "}")))
          (1 font-lock-variable-name-face prepend))
         ;; Ineffective backslashes (typically in need of doubling).
         ("\\(\\\\\\)\\([^\"\\]\\)"
          (1 (elisp--font-lock-backslash) prepend))
         ;; Words inside ‘’, '' and `' tend to be symbol names.
         (,(concat "[`‘']\\(" (rx macl-mode-symbol) "\\)['’]")
          (1 font-lock-constant-face prepend))
         ;; \\= tends to be an escape in doc strings.
         (,(rx "\\\\=")
          (0 font-lock-builtin-face prepend))
         ;; Constant values.
         (,(lambda (bound) (macl-mode--search-key ":" bound))
    ...
    (defconst lisp-cl-font-lock-keywords-2
      (append
       ...
       `( ;; Regexp negated char group.
         ...
         (,(concat "#:" (rx macl-mode-symbol) "") 0 font-lock-builtin-face)
         ;; Constant values.
         (,(lambda (bound) (macl-mode--search-key ":" bound))
          (0 font-lock-builtin-face))
         ;; ELisp and CLisp `&' keywords as types.
         ...
         (,(concat "(\\(\\(do-\\|with-\\)" (rx macl-mode-symbol) "\\)")
           (1 font-lock-keyword-face))
         (lisp--match-hidden-arg
          (0 '(face font-lock-warning-face
               help-echo "Easy to misread; consider moving the element to the next line")
...
(defun macl-mode-variables (&optional lisp-syntax keywords-case-insensitive
                                      elisp)
...
font-lock keywords will not be case sensitive."
  (when lisp-syntax
    (set-syntax-table macl-mode-syntax-table))
...
(defun lisp-outline-level ()
  ...
  (let ((len (- (match-end 0) (match-beginning 0))))
    (cond ((or (looking-at-p "(")
               (looking-at-p macl-mode-autoload-regexp))
           1000)
          ((looking-at ";;\\(;+\\) ")
...
(defvar-keymap macl-mode-shared-map
...
(defvar-keymap macl-mode-map
  :doc "Keymap for ordinary Lisp mode.
All commands in `macl-mode-shared-map' are inherited by this map."
  :parent macl-mode-shared-map
  "C-M-x"   #'lisp-eval-defun
  "C-c C-z" #'run-lisp)

(easy-menu-define macl-mode-menu macl-mode-map
...
(define-derived-mode macl-mode lisp-data-mode "MACL"
...
(defalias 'common-macl-mode 'macl-mode)
	    ...
	    (paragraph-separate
	     (concat paragraph-separate "\\|\\s-*\".*[,\\.]$"))
            (fill-column (if (and (integerp emacs-lisp-docstring-fill-column)
                                  (derived-mode-p 'emacs-macl-mode))
                             emacs-lisp-docstring-fill-column
                           fill-column)))
        (let ((ppss (syntax-ppss))
              (start (point))
              ;; Avoid recursion if we're being called directly with
              ;; `M-x lisp-fill-paragraph' in an `emacs-macl-mode' buffer.
